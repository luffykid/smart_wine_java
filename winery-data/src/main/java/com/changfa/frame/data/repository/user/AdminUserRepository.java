...
public interface AdminUserRepository extends AdvancedJpaRepository<AdminUser,Integer> {
    ...
    AdminUser findAdminUserByPhone(String phone,Long wineryId);
    ...
    List<AdminUser> findAdminUserByWId(Long wineryId, String search);
    ...
    List<AdminUser> findAdminUserByWIdAndRoleName(Long wineryId, String roleName);

    //获取酒庄的超级管理员用户
    @Query("select a from AdminUser a where a.wineryId = ?1 and a.isSuper = 'Y' ")
    AdminUser findAdminUserByWineryId(Long wineryId);


    /*酒庄下的工作人员数量*/
    int countAdminUsersByWineryIdAndStatus(Long wineryId,  String status);
