...
public class FileUtil {
     ...
     */
    public static void deleteNFSByFileUrl(String orgFileUrl) {
        try {
            //不一致说明图片已被修改
            if (StringUtils.isNotBlank(orgFileUrl)) {
     ...
     * 图片合成
     *
     * @param baseImgUrl   底图地址【公网地址】
     * @param renderImgUrl 渲染图地址【公网地址】
     * @param renderScaleX x轴比例
     * @param renderYScale y轴比例
     * @param alpha        透明度, 【0.0~1.0: 完全透明~完全不透明】
     * @return
     */
    public static String getUrlOfComposite(String baseImgUrl, String renderImgUrl, BigDecimal renderScaleX, BigDecimal renderYScale, float alpha, String targetDir) {
        // 获取底图
        try {
            // 获取底图和渲染图buffer
            BufferedImage baseImg = ImageIO.read(new URL(baseImgUrl));
            BufferedImage renderImg = ImageIO.read(new URL(renderImgUrl));

            // 创建Graphics2D对象，用在底图对象上绘图
            Graphics2D g2d = baseImg.createGraphics();

            // 获取渲染图宽高
            int waterImgWidth = renderImg.getWidth();
            int waterImgHeight = renderImg.getHeight();

            // 计算渲染图在底图X、Y的距离
            BigDecimal baseImgWidth = new BigDecimal(baseImg.getWidth());
            BigDecimal baseImgHeight = new BigDecimal(baseImg.getHeight());
            int renderX = baseImgWidth.multiply(renderScaleX).intValue();
            int renderY = baseImgHeight.multiply(renderYScale).intValue();

            // 设置底图透明效果并绘图
            g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, alpha));
            g2d.drawImage(renderImg, renderX, renderY, waterImgWidth, waterImgHeight, null);
            ...
            // 保存文件的目标文件夹，基于项目根路径
            String saveDir = targetDir + "/" + DateUtil.convertDateToStr(new Date(), "yyyyMMdd");
            String destDir = PropConfig.getProperty(PropAttributes.NFS__SERVICE_FILE_SHARE_PATH) + saveDir;
            ...
            // 目标文件, 生成文件名
            String suffix = baseImgUrl.substring(baseImgUrl.lastIndexOf(".") + 1);
            String baseName = RandomStringUtils.randomAlphanumeric(12);
            String newFileName = baseName + "." + suffix;
            File destFile = new File(destDir + "/" + newFileName);

            // 生成合成后的文件destFile
            ImageIO.write(baseImg, suffix, destFile);
            ...
            sb.append("/");
            sb.append(newFileName);
            String nfsUrl = sb.toString();

            // 返回文件绝对路径
            if (StringUtils.isNotEmpty(nfsUrl) && !nfsUrl.startsWith("http")) {
                return PropConfig.getProperty(PropAttributes.NFS_SERVICE_FILE_SERVER) + nfsUrl;
            }
            return nfsUrl;
